...
class ItemView(tk.Frame):
    ...
    def __init__(self, master: tk.Frame, item_name: str, amount: int,
        ...
        self.bind_all("<Button-1>", self.select_command)
...
class FarmGame:
    ...
    def __init__(self, master: tk.Tk, map_file: str) -> None:
        ...
        # Item View Class
        for items in ITEMS:
            self.items_view = ItemView(middle_frame, items,
            ...
            self.items_view.pack(side=tk.TOP, expand=True, fill=tk.BOTH)
        ...
        def load_map_file():
            self.map_file = filedialog.askopenfilename(initialdir="maps",
                                                       title="Select map file",
                                                       filetypes=[("text file", "*.txt")])
            if self.map_file:
                self.master.destroy()  # Destroy the current window
                root = tk.Tk()  # Create root window
                farm_game = FarmGame(root, self.map_file)  # Start new game with new map
    ...
    def clear_cache(self):
        """
        Clear the cache before starting the new game.
        """
        self.cache = {}
    ...
    def redraw(self) -> None:
        ...
        for items in ITEMS:
            self.items_view.update(self.farm_model.get_player().get_inventory().get(items, 0),selected = True)
    ...
    def handle_keypress(self, event: tk.Event) -> None:
        ...
        # Plant the selected item
            ...
            if selected_item is not None and len(selected_item) == 2:
                self.farm_model.add_plant(self.farm_model.get_player_position(),
                                          globals()[selected_item[0] + 'Plant']())
            self.redraw()
        # Harvest the plant
        elif key == "h":
            player = self.farm_model.get_player()
